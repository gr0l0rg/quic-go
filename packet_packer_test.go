...
var _ = Describe("Packet packer", func() {
	...
	expectAppendStreamFrames := func(frames ...ackhandler.Frame) {
		framer.EXPECT().AppendStreamFrames(gomock.Any(), gomock.Any(), protocol.Version1).DoAndReturn(func(fs []ackhandler.Frame, _ protocol.ByteCount, v protocol.VersionNumber) ([]ackhandler.Frame, protocol.ByteCount) {
	...
	Context("packing packets", func() {
		...
		Context("packing ACK packets", func() {
			...
			It("packs 1-RTT ACK-only packets, after handshake confirmation", func() {
				...
				ack := &wire.AckFrame{AckRanges: []wire.AckRange{{Smallest: 1, Largest: 10}}}
				ackFramer.EXPECT().GetAckFrame(protocol.Encryption1RTT, true).Return(ack)
				p, err := packer.PackPacket(true, protocol.Version1)
				Expect(err).NotTo(HaveOccurred())
				Expect(p).ToNot(BeNil())
		...
		Context("packing 0-RTT packets", func() {
			...
			It("packs a 0-RTT packet", func() {
				...
				framer.EXPECT().HasData().Return(true)
				framer.EXPECT().AppendControlFrames(gomock.Any(), gomock.Any(), protocol.Version1).DoAndReturn(func(frames []ackhandler.Frame, _ protocol.ByteCount, _ protocol.VersionNumber) ([]ackhandler.Frame, protocol.ByteCount) {
		...
		Context("packing normal packets", func() {
			It("returns nil when no packet is queued", func() {
				...
				p, err := packer.PackPacket(false, protocol.Version1)
				Expect(p).To(BeNil())
				Expect(err).ToNot(HaveOccurred())
			})
			...
			It("accounts for the space consumed by control frames", func() {
				...
				_, err := packer.PackPacket(false, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
			})
			...
			It("pads if payload length + packet number length is smaller than 4", func() {
				...
				expectAppendControlFrames()
				expectAppendStreamFrames(ackhandler.Frame{Frame: f})
				packet, err := packer.PackPacket(false, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
				// cut off the tag that the mock sealer added
				packet.buffer.Data = packet.buffer.Data[:packet.buffer.Len()-protocol.ByteCount(sealer.Overhead())]
				hdr, _, _, err := wire.ParsePacket(packet.buffer.Data, packer.getDestConnID().Len())
				...
				l, frame, err := frameParser.ParseNext(packet.buffer.Data[len(data)-r.Len():], protocol.Encryption1RTT, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("packing probe packets", func() {
			for _, pers := range []protocol.Perspective{protocol.PerspectiveServer, protocol.PerspectiveClient} {
				...
				It(fmt.Sprintf("packs an Initial probe packet and pads it, for the %s", perspective), func() {
					...
					packet, err := packer.MaybePackProbePacket(protocol.EncryptionInitial, protocol.Version1)
					Expect(err).ToNot(HaveOccurred())
					Expect(packet).ToNot(BeNil())
			...
			It("packs a Handshake probe packet", func() {
				...
				packet, err := packer.MaybePackProbePacket(protocol.EncryptionHandshake, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
				Expect(packet).ToNot(BeNil())
			...
			It("packs a full size 1-RTT probe packet", func() {
				...
				framer.EXPECT().HasData().Return(true)
				expectAppendControlFrames()
				framer.EXPECT().AppendStreamFrames(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(func(fs []ackhandler.Frame, maxSize protocol.ByteCount, v protocol.VersionNumber) ([]ackhandler.Frame, protocol.ByteCount) {
			...
			It("packs an MTU probe packet", func() {
				...
				p, err := packer.PackMTUProbePacket(ping, probePacketSize, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
